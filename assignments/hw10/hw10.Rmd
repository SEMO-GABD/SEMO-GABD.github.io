...
**Grad and Honors:** Recreate this figure using density plots. However, modify the right panel so that all three months are overlaid on the same plot. Go back to [the notes](../../notes/notes10.html) and follow the example code for guidance. Ensure that the guide legend `Month` shows *only* for color, not alpha or other mappings.
...
Writing the code for dozens of scatterplots could grow tedious quickly, even with copy and paste. You have to ensure you changed each of the variables correctly. Fortunately, the [GGally](http://ggobi.github.io/ggally/) package provides the <!--[ggscatmat](http://ggobi.github.io/ggally/articles/ggscatmat.html) and --> [ggpairs](http://ggobi.github.io/ggally/articles/ggpairs.html) function. <!--Before you go farther, scroll down to browse through the examples for this function. `ggscatmat()` creates only a scatterplot matrix, which we will use. `ggpairs()` can produce many more types of plots in the matrix, as you saw looking through the examples.-->

We will use the `columns` argument of `ggpairs` to specify the first seven columns of data. If you want to plot all columns (helpful but not always wise), you do not have to supply the `columns` argument.
...
```{r}
sparrows %>% ggpairs(columns = 1:7)
```

`ggpairs` includes a density plot on the diagonal and puts the scatterplots below the diagonal, but we otherwise recreated Figure 10 with a single line of code. Niiiice.
