---
title: "HW 05: Data Importing and Wrangling 1"
...
- arrange, filter, and select the imported data;
- use pipes; and
...
- Create a `data` folder in the same folder as your `Rproj` file, at the same level as your other `hw` folders. Do *not* put this folder in your `hw05` folder.
...
- Download this [R Notebook file](https://raw.githubusercontent.com/SEMO-GABD/semo-gabd.github.io/master/assignments/hw05/worksheet_hw05a.Rmd). Right-click to save the file as `<lastname>_hw05a.Rmd` inside the "hw05" folder.

- Install the `nycflights13` package. You will need it to complete this assignment and the next.
...
- Create a new notebook file called `<lastname>_05b.Rmd`. Edit the YAML file as you have for previous assignments.
- Load the `tidyverse` package.
- In this same code chunk, you can define a variable with the path to your `data` folder, as discussed in the notes.
- [Download this zip file](https://github.com/SEMO-GABD/semo-gabd.github.io/raw/master/assignments/hw05/data.zip) of data. Unzip the file and place all of the files in your `data` folder. If you find yourself asking, "What data folder?", then (1) shame on you for not reading the notes and (2) go read the notes.
...
```{r}
terns <- read_tsv(file.path(file_path, "roseate_terns.txt"))

terns <- terns %>% 
  gather(key = pop, value = size, -1)
...
*Put your data import and wrangling chunk here. Do not include this line.*
...
darter <- read_fwf(file.path(file_path, "darters.txt"), 
                   skip = 13,
                   fwf_widths(widths,
                   col_names = names))
...
- Data: abalone.csv (ab-ah-LOW-knee; rhymes bologne)
