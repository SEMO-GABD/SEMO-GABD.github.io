...
Type a `#` followed by your name. R uses # sign to add comments, so that you and others can understand what the R code is about.  Comments are not run as R code, so they will not influence your result. For example, the code below shows a comment ("Calculate 2 + 5"), followed by the actual code, then another comment. Only the code is executed by R.  
...
Add a line of new line code in your script that calculates the sum of 30 and 12. Run the line of code. Write a comment above your line that describes what your code does.
...
You would not normally write a comment for every step. The code to add 30 + 12 is obvious. For now, though, write comments for most steps just to develop the habit.
