---
title: "05: Data Importing and Wrangling I"
...
You have to tell R where your data is located to import it. The location, or path, of a file is handled somewhat differently between Macs and PCs. Macs use the unix system. PCs use the DOS system. 

- Mac: `~/Documents/folder1/folder2/file.txt`
...
From here on, you will import data from files stored on your computer. I suggest you create a `data` folder inside the same folder as your `Rproj` file. Put all of your data files for this course in your `data` folder so you can access them as needed for any assignment.
...
```{r eval=FALSE}
dat <- read_csv("~/Documents/bi485/data/my_file.csv")  # Mac or
dat <- read_csv("C:/Documents/bi485/data/my_file.csv") # PC
```
...
`dat <- read.csv(file.path(file_path, your_file))`

The first function to run is the inner function, `file.path()` because it has to assemble the path and file name. `file.path()` then passes the result to `read.csv()`. You could so the same thing like this:
...
**Note:** Instead of typing a space, `%`, then `>`, then `%`,  and then another space, type `cmd/alt + shift + m` to insert the whole pipe symbol at once!
...
The trick to remember gathering is that the `key` argument will make a column filled with the column names; the `value` argument will make a column filled with the data that originally were in the separate columns. See Figure 12.2 of [R4ds](https://r4ds.had.co.nz/tidy-data.html#spreading-and-gathering).
