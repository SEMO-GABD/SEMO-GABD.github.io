...
Read the notes below and complete the assignment. Then refer back to these resources as reference.
...
<!--You used R Notebooks in [HW02](http://../hw/hw02.html). -->
R Notebooks are interactive documents that allows you to use the markdown commands that you learned in [HW01](http://../hw/hw01.html). R Notebooks use **R Markdown**, an enhanced version of markdown with additional features. These features allow you to produce reports that include R code, tables and graphs from your analyses in RStudio. You can use R Markdown and RStudio together to perform analyses that can be shared and reproduced by anyone. These tools have even been used to write [thesis and dissertations!](http://tysonbarrett.com/jekyll/update/2018/02/11/r_dissertation/)

For this course, you will use R Notebook documents for many of your assignments, starting now. To use R Notebooks, you must install the `rmarkdown` package. In RStudio, choose `Tools > Install Packages...` from the menu. Enter `rmarkdown` into the space under `Packages`, be sure `Install dependencies` is checked, and then press the "Install" button. Installing `rmarkdown` will install several additional packages so installation may take a few minutes. *You only have to do this once.*

After the packages are isntalled, choose `File > New File > R Notebook` from the menu.  RStudio creates a new document with example R Markdown text. You will see familiar markdown commands, like `##` for section headers and `** … **` for **bold text.** 
...
This is an R code chunk. A code chunk begins with three back ticks ` ```{r} ` on a line, followed by your R code, then three more back ticks ` ``` ` to end the chunk. When you make your document, the code will be evaluated in R, and the output will be included after the chunk (as specified by the `editor_options:` in the YAML header). In the case of `summary(iris)`, the `summary()` function calculates basic statistics for each variable in the data from the `iris` data set, which is then shown below the chunk.
